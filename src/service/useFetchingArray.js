...
import {TIMEOUT} from "./fetch_service.js"
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            const initialResults = urls.map(url => ({
                url: url.url, data: null, filial: url.filial, loading: true, error: null, params: url.params,
            }))
                ...
                try {
                    const response = await axios.get(url.url, {
                        timeout: TIMEOUT, headers: {
                            Authorization: token, uid_filial: url.filial.uid, wp, kiosk, version
                        }, params: url.params,
                    })
                    return {
                        url: url.url,
