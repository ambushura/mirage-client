...
import { ORDER_TIME_REMAINING, setPreOrderTimeRemaining } from '../../../../../redux/desktop/frontoffice/ordersReducer.js'
...
const PageSeance = () => {
    ...
    if (seance !== undefined && hall !== null) {
        if (kiosk_checkout === 0) {
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column' }}>
                        ...
                        <Box id="content" style={{ display: 'flex', flexDirection: 'row', justifyContent: 'space-between' }}>
                            <Box
                                ...
                                sx={{ flex: 1 }}
                            >
                                <Hall city={city} filial={filial} pre_order={pre_order} hall={hall} seance={seance} booking={booking} />
                            </Box>
                            <Box sx={{ minWidth: kiosk || user === null ? 0 : 'var(--order-width)' }}>
                                <Order />
                            </Box>
                        </Box>
            ...
            return (
                <Box style={{ display: kiosk_checkout ? 'block' : 'none', height: '100%' }}>
                    <CheckOut />
                </Box>
            )
        }
