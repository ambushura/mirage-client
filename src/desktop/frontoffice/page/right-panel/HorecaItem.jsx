...
import { selectUidHoreca } from '../../../../redux/desktop/frontoffice/ordersReducer.js'
...
const HorecaItem = (props) => {
    ...
    const state = [
        <></>,
        <Box color="primary" key="1">
            Начать готовить
        </Box>,
        <Box color="primary" key="2">
            Закончить готовку
        </Box>,
        <Box color="primary" key="3">
            Отдать гостю
        </Box>,
    ]
        ...
        >
            <Box className="order-box-horeca-item-1">
                ...
                <Box
                    ...
                    onClick={() => {
                        ...
                        dispatch(
                            selectUidHoreca(
                                uid_selected.includes(props.item.uid)
                                    ? uid_selected.filter((el) => el !== props.item.uid)
                                    : [...uid_selected, props.item.uid]
                            )
                        )
                    }}
                >
            ...
            {props.item.mark !== null ? (
                <Box className="order-box-horeca-item-2">
                    ...
                    >
                        {props.item.mark.value === null ? 'Отсканируйте маркировку' : props.item.mark.value}
                    </Box>
                    <Button variant="text" color="secondary">
            ...
            {props.item.kitchen !== null && props.item.kitchen.modifications !== null && props.item.kitchen.modifications.length > 0 ? (
                <Box className="modifications">
                    {props.item.kitchen.modifications.map((modification) => {
                        return (
                            <Box className="modification" key={modification.uid}>
                                {modification.name}
                            </Box>
                        )
                    })}
                ...
                <Box className="order-box-horeca-item-5">
                    ...
                    {props.item.kitchen.uid_delivery_path !== null ? (
                        <Button variant="text" color="info">
                            {props.item.kitchen.name_delivery_path}
                        </Button>
                    ) : null}
                </Box>
            ) : null}
