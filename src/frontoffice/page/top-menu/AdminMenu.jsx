        ...
        <Box sx={{ marginRight: '5px', display: 'flex', flexDirection: 'row', alignItems: 'center' }}>
            <FormControl variant="filled" sx={{ minWidth: '200px' }}>
                ...
                >
                    {halls !== null &&
                        halls.map((hall) => (
                            <MenuItem key={hall.uid} value={hall.uid}>
                                {hall.title}
                            </MenuItem>
                        ))}
                </Select>
            ...
            <ButtonGroup variant="contained">
                <Button
                    color={mode === 'block' ? 'primary' : 'secondary'}
                    onClick={() => {
...
export function DateParamAdmin() {
    ...
    const handleOnChange = async (value) => {
        ...
        await navigate(
            `/${current_page}/${city.code}/${filial === undefined ? 'all' : filial.eais}/${current_param_data}/${current_page === 'film' ? film.uid + '/' : ''}?${wp !== null ? 'wp=' + wp : ''}${kiosk ? '&kiosk' : ''}`
        )
            ...
            <ButtonGroup size="medium" variant="contained" color="secondary" className="admin-panel-period" sx={{ marginRight: '5px' }}>
                ...
                <Button
                    onClick={async () => {
                        ...
                        await navigate(
                            `${city !== undefined ? `/${current_page}/${city.code}/${filial === undefined ? 'all' : filial.eais}/${date}` : '/'}?${wp !== null ? 'wp=' + wp : ''}${kiosk ? '&kiosk' : ''}`
                        )
                        await dispatch(setCurrentPreOrder(NEW_EMPTY_ORDER()))
                        await dispatch(setCurrentHorder(NEW_EMPTY_HORDER()))
                        dispatch(setOrdersHorecaPage(1))
...
export function ReportsMenu() {
    ...
    return (
        <ButtonGroup variant="contained" color="secondary">
            ...
            <Button
                color={report_variant === 'attendance' ? 'primary' : 'secondary'}
                onClick={() => dispatch(setReportVariant('attendance'))}
            >
                Посещаемость
            </Button>
