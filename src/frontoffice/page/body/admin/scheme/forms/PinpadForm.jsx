...
export default function PinpadForm({props}) {
    ...
    const onSubmit = (data) => {
        ...
        if (prepared.date_change)
            prepared.date_change = dayjs(prepared.date_change).format('YYYY-MM-DDTHH:mm:ss+00:00')
    }
    ...
    useEffect(() => {
        set_fast_commands([
            {id: 0, name: 'Проверка связи с терминалом', route: '', param: {uid}},
            {
                id: 1,
                name: 'Проверка связи с хостом',
                route: '',
                param: {uid},
            },
            {id: 2, name: 'Оплата/Отмена (1 рубль)', route: '', param: {uid}},
            {
                id: 3,
                name: 'Сверка итогов',
                route: ROUTE_EQUIPMENT_PINPAD_Z,
                param: {uid},
            },
        ])
    }, [uid])
