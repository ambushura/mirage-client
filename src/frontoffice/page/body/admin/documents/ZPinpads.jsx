...
const ZPinpads = () => {
    ...
    const [fetching_zpinpads, set_fetching_zpinpads] = useState({
        loading: false,
        error: null,
        data: null,
    })
    ...
    useEffect(() => {
        const fetch = async () => {
            const fetching_result = await dispatch(
                common_documents_pinpads_get(filial, param_date_admin, zpinpads_update)
            )
    ...
    useEffect(() => {
        const fetch = async () => {
            const fetching_result = await dispatch(
                common_documents_slips_get(filial, param_date_admin, uid_pinpad_current)
            )
        ...
        return (
            <Box sx={{height: '100%', display: 'flex', flexDirection: 'column'}}>
                ...
                {!fetching_slips.loading && fetching_slips.error !== null && (
                    <Box sx={{minHeight: '50%'}}>
                        <Box sx={{minHeight: '50%'}} className="empty-box">
                            {fetching_slips.error}
                        </Box>
                    </Box>
                )}
                {!fetching_slips.loading &&
                    fetching_slips.error === null &&
                    fetching_slips.data !== null &&
                    slips !== undefined && (
                        <Box sx={{minHeight: '50%'}}>
                            {slips.length > 0 ? (
                                <DataGridPro
                                    ...
                                    onRowDoubleClick={(params) => {
                                        navigate(
                                            `/admin/slip/${city.code}/${filial.eais}/${params.row.id}/?${wp !== null ? 'wp=' + wp : ''}`
                                        )
                                    }}
                                />
                            ) : (
                                <Box className="empty-box" sx={{height: '100%'}}>
                                    Слипы отсутствуют в смене...
                                </Box>
                            )}
                        </Box>
                    )}
                ...
                {!fetching_zpinpads.loading && fetching_zpinpads.error !== null && (
                    <Box sx={{minHeight: '50%'}}>
                        <Box sx={{minHeight: '50%'}} className="empty-box">
                            {fetching_zpinpads.error}
                        </Box>
                    </Box>
                )}
                {!fetching_zpinpads.loading &&
                    fetching_zpinpads.error === null &&
                    fetching_zpinpads.data !== null &&
                    zpinpads !== undefined && (
                        <Box sx={{minHeight: '50%'}}>
                            {zpinpads.length > 0 ? (
                                <DataGridPro
                                    ...
                                    onRowDoubleClick={(params) => {
                                        navigate(
                                            `/admin/z_acquiring/${city.code}/${filial.eais}/${params.row.id}/?${wp !== null ? 'wp=' + wp : ''}`
                                        )
                                    }}
                                />
                            ) : (
                                <Box className="empty-box" sx={{height: '100%'}}>
                                    Итоговые операции эквайринга в смене отсутствуют...
                                </Box>
                            )}
                        </Box>
                    )}
...
export const columns_zpinpads = [
    {field: 'id', headerName: 'UID документа', width: 10},
    {
...
export const columns_slips = [
    ...
    {
        field: 'print_error',
        headerName: 'Ошибка печати',
        width: 100,
    },
    {field: 'slip0', headerName: '0', width: 100},
    {field: 'slip4', headerName: '4', width: 100},
    {
        field: 'slip6',
        headerName: '6',
        width: 100,
    ...
    {field: 'slip9', headerName: '9', width: 100},
    {field: 'slip10', headerName: '10', width: 100},
    {
        field: 'slip11',
        headerName: '11',
        width: 100,
