...
const Zbooks = () => {
    ...
    useEffect(() => {
        const fetch = async () => {
            const fetching_result = await dispatch(
                common_documents_zbooks_get(filial, param_date_admin, zbooks_update)
            )
    ...
    useEffect(() => {
        const fetch = async () => {
            const fetching_result = await dispatch(
                common_documents_receipts_get(filial, param_date_admin, uid_kkt_current)
            )
        ...
        return (
            <Box sx={{height: '100%', display: 'flex', flexDirection: 'column'}}>
                {fetching_receipts.loading &&
                    ...
                    fetching_receipts.data === null && <Loader/>}
                {!fetching_receipts.loading &&
                    fetching_receipts.error !== null &&
                            ...
                            {receipts.length > 0 ? (
                                <DataGridPro
                                    ...
                                    onColumnVisibilityModelChange={(newModel) =>
                                        setColumnVisibilityModelReceipts(newModel)
                                    }
                                    sx={{
                                        '& .total-row': {
                ...
                {!fetching_zbooks.loading &&
                    fetching_zbooks.error === null &&
                    fetching_zbooks.data !== null &&
                    zbooks !== undefined && (
                        <Box sx={{minHeight: '50%'}}>
                            {zbooks.length > 0 ? (
                                <DataGridPro
                                    ...
                                    onColumnVisibilityModelChange={(newModel) =>
                                        setColumnVisibilityModelZBooks(newModel)
                                    }
                                    pinnedColumns={{
                                    ...
                                    onRowDoubleClick={(params) => {
                                        navigate(
                                            `/admin/zbook/${city.code}/${filial.eais}/${params.row.id}/?${wp !== null ? 'wp=' + wp : ''}`
                                        )
                                    }}
                                />
                            ) : (
