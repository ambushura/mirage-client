...
const Receipt = () => {
    ...
    useEffect(() => {
        if (trigger_submit_receipt) {
            ...
            dispatch(
                addNotification({
                    message: `Кассовый чек ${uid === 'new' ? '' : ' №' + number} сохранен`,
                    severity: 'info',
                    autoHide: true,
                })
            )
    ...
    const onSubmit = (data) => {
        ...
        if (prepared.moment) prepared.moment = dayjs(prepared.moment).format('YYYY-MM-DDTHH:mm:ss+00:00')
    ...
    useEffect(() => {
        dispatch(
            setCaptionReceipt(
                `КАССОВЫЙ ЧЕК ${uid === 'new' ? ' * ' : ' №' + number + ' от ' + dayjs(date_shift).format('DD.MM.YY') + ' ЗН ' + number_kkt}`
            )
        )
        return () => {
                    ...
                    >
                        ...
                        >
                            ...
                            <ControlledLazySelect
                                ...
                                optionsStatic={[
                                    { uid: '0', title: 'Основная' },
                                    { uid: '1', title: 'Упрощенная' },
                                ]}
                            />
                            <ControlledLazySelect
                                control={control}
                        ...
                        >
                            ...
                            />
                            <ControlledMoneyField control={control} name="price" label="Цена" rules={{ required: 'Введите цену' }} />
                            <ControlledMoneyField control={control} name="sum_discount" label="Сумма скидки" />
                            <ControlledMoneyField control={control} name="sum" label="Сумма со скидкой" readOnly={true} />
                            <ControlledSwitch control={control} name="copy" label="Копия" />
                        </Box>
                        <Box
                            sx={{
                                display: 'flex',
                        ...
                        >
                            ...
                            />
                            <ControlledSwitch control={control} name="printed" label="Напечатан" color="secondary" />
                            <Box sx={{ backgroundColor: '#aee3ff', padding: '10px', borderRadius: '12px' }}>
                                <ControlledSwitch
                                    ...
                                    color="primary"
                                />
                                <ControlledDatePicker control={control} name="date_shift_confirm" label="Дата смены фактическая" />
                            </Box>
                        </Box>
                    </Box>
                    <Box sx={{ flex: 1 }}>
                        <ControlledTextField control={control} name="comment" label="Комментарий" sx={{ width: '100%' }} />
                    </Box>
                    <Box>
                        <DataGridPro
                            hideFooter
...
export const columns_items = [
    { field: 'id', headerName: 'Номер строки', width: 10 },
    {
    ...
    {
        field: 'mark_kkt_check_result',
        headerName: 'ЧЗ КП',
        width: 130,
    },
    { field: 'mark_req_id', headerName: 'ЧЗ ИД РР', width: 50 },
    {
        field: 'mark_req_timestamp',
        headerName: 'ЧЗ Дата проверки РР',
        width: 50,
    },
