...
const SeanceCard = (props) => {
    ...
    return (
        <NavLink to={`/seance/${city.code}/${filial.eais}/${seance.uid}?${wp !== null ? 'wp=' + wp : ''}${kiosk ? '&kiosk' : ''}`}>
            <div className="seances-body-seance">
                ...
                >
                    <div className="seances-body-seance-description-time">
                        {String(beginning.$H).padStart(2, '0')}:{String(beginning.$m).padStart(2, '0')}
                    </div>
                    <div className="seances-body-seance-description-copy-type">{seance.copy_type}</div>
                </div>
                <div className="seances-body-seance-price">от {min_tariff(seance)} р</div>
            </div>
        </NavLink>
    )
}
