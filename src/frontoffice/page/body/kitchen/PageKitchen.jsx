                            ...
                            <Box className="kitchen-order-header glass">
                                ...
                                <Box sx={{flex: 1, paddingLeft: '12px', overflow: 'hidden'}}>
                                    <Box sx={{fontWeight: '500'}}>
                                        создан в {dayjs.utc(order.date_create).format('HH:mm')}
                                    </Box>
                                    {showTimer && (
                                        <Box sx={{color: '#8B919B'}}>
                            ...
                            <Box className="kitchen-order-body">
                                {order.items.map((item, i) => (
                                    <Box
                                        ...
                                        style={{
                                            borderBottom:
                                                i !== order.items.length - 1 && !item.canceled
                                                    ? '1px dashed #b1b1b7'
                                                    : 'none',
                                        }}
                                    >
                                        ...
                                        >
                                            ...
                                            >
                                                {item.take_away && (
                                                    <Box sx={{fontSize: '120%', marginRight: '5px'}}>С СОБОЙ</Box>
                                                )}
                                                {item.course > 0 && (
                                                    <Box sx={{fontSize: '120%'}}>{item.course + 1} КУРС</Box>
                                                )}
                                            </Box>
                                            <Box
                                                sx={{
                                                ...
                                                >
                                                    {item.modifications.map((modification) => {
                                                        return (
                                                            <Box
                                                                key={modification}
                                                                sx={{fontWeight: 'bold', padding: '4px 4px 0 0'}}
                                                            >
                                                                {modification}
                                                            </Box>
                                                        )
    ...
    useEffect(() => {
        const fetch = async () => {
            const fetching_result = await dispatch(
                horeca_kitchen_get(filial, param_date_admin, uid_kitchen_points_selected)
            )
        ...
        <Box id="content-box" sx={{overflowY: 'auto'}}>
            <Box sx={{display: 'flex', flexDirection: 'column'}}>
                ...
                {!fetching.loading && fetching.error === null && fetching.data !== null && (
                    <Box
                        id="content"
                        sx={{height: 'calc(var(--page-height) - var(--footer-height)) !important'}}
                    >
                        {kitchen_orders !== null &&
                        (kitchen_orders.cooking.length > 0 || kitchen_orders.completed.length > 0) ? (
                            <>
