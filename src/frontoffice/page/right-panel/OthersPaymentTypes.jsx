    ...
    useEffect(() => {
        const fetch = async () => {
            let fetching_result = await dispatch(
                common_payment_methods_get(filial, props.order.uid, props.type, true)
            )
            if (
        ...
        <Box>
            ...
            >
                ...
                payment_methods.data === null ? (
                    <Box className="empty-box">Ошибка загрузки маршрутов оплаты {payment_methods.error}</Box>
                ) : null}
                {payment_methods.loading &&
                payment_methods.error === null &&
                payment_methods.data === null ? (
                                        ...
                                        chapter2_array.forEach((chapter2) => {
                                            props.order[chapter0][chapter1][chapter2].forEach((item) => {
                                                if (!pm.hidden) {
                                                    ...
                                                    if (
                                                        payment_group[chapter0][chapter1][chapter2].items.includes(item.uid)
                                                    ) {
                                                        if (item.name_payment_type === null) {
                                                            if (
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            {!['Отложенная оплата', 'На расчетный счет'].includes(pm.name) && (
