...
const Payment = (props) => {
    ...
    useEffect(() => {
        const fetch = async () => {
            let fetching_result = await dispatch(
                common_payment_methods_get(filial, props.order.uid, props.type, false)
            )
            if (
        ...
        chapter0_array.forEach((chapter0) => {
            chapter1_array.forEach((chapter1) => {
                chapter2_array.forEach((chapter2) => {
                    if (
                        payment_group[chapter0][chapter1][chapter2].count > 0 &&
                        payment_group[chapter0][chapter1][chapter2].items.length > 0
                    ) {
                        show = true
                    }
                })
                ...
                <Box sx={{display: 'flex', flexDirection: 'row', width: '100%'}}>
                    ...
                    <FormControlLabel
                        sx={{
                            ...
                            borderBottom: '1px #878787 solid',
                        }}
                        control={<Switch checked={return_before} onChange={handleReturnBefore}/>}
        ...
        const grouped_items = useMemo(() => {
            ...
            switch (chapter0) {
                case 'for_payment':
                    return groupAndSum(
                        group,
                        ['name', 'unit_name', 'price', 'sum_discount', 'uid_discount', 'name_payment_type'],
                        ['quantity', 'sum']
                    )
                case 'for_returning':
                    return groupAndSum(
                    ...
                    {chapter1 !== 'success' ? (
                        <Checkbox
                            ...
                            onChange={() => {
                                ...
                                if (action) {
                                    payment_group_new[chapter0][chapter1][chapter2].items = grouped_items.flatMap(
                                        (item) => item.__uids
                                    )
                                } else {
                    ...
                    {grouped_items.length > 0 && (
                        ...
                        >
                            {grouped_items.map((item, i) => (
                                <motion.div key={item.__uids.join('-')} variants={itemVariants}>
                                    <Box
                                        style={{
                                            borderBottom: i !== grouped_items.length - 1 ? '1px dashed #b6b5b5' : null,
                                        }}
                                    >
                                        <Box className="payment-items-group-item-row">
                                            {chapter1 !== 'success' ? (
                                                <Checkbox
                                                    checked={item.__uids.every((uid) =>
                                                        payment_group[chapter0][chapter1][chapter2].items.includes(uid)
                                                    )}
                                                    onChange={() => {
                                                        const payment_group_new = structuredClone(payment_group)
                                                        const selectedItems =
                                                            payment_group_new[chapter0][chapter1][chapter2].items
                                                        const allSelected = item.__uids.every((uid) =>
                                                            selectedItems.includes(uid)
                                                        )
                                                        if (allSelected) {
                                                            payment_group_new[chapter0][chapter1][chapter2].items =
                                                                selectedItems.filter((uid) => !item.__uids.includes(uid))
                                                        } else {
                                                            payment_group_new[chapter0][chapter1][chapter2].items = [
                                                                ...new Set([...selectedItems, ...item.__uids]),
                                                            ]
                                                        }
                                                        const totalUids = grouped_items.flatMap((g) => g.__uids)
                                                        const selectedUids =
                                                            payment_group_new[chapter0][chapter1][chapter2].items
                                                        payment_group_new[chapter0][chapter1][chapter2].selected =
                                                            totalUids.every((uid) => selectedUids.includes(uid))
                                                        set_payment_group(payment_group_new)
                                                    }}
                                                />
                                            ) : null}
                          ...
                          {item.name_return_reason !== null &&
                              item.name_return_reason.toLowerCase()}
                        </span>
                                                {item.comment_return_reason !== null && (
                                                    <span>, {item.comment_return_reason}</span>
                                                )}
                                            </Box>
                                        )}

                                        {item.name_payment_type !== null ? (
                                            <Box
                                                sx={{
                                                    fontSize: '80%',
                                                    color: 'grey',
                                            ...
                                            >
                                                ...
                                                {item.uid_in_receipt !== null ? (
                                                    <Box
                                                        ...
                                                        onClick={() => {
                                                            navigate(
                                                                `/admin/receipt/${city.code}/${filial.eais}/${item.uid_in_receipt}/?${wp !== null ? 'wp=' + wp : ''}`
                                                            )
                                                        }}
                                                    >
                                                        Чек приход
                                                ...
                                                {item.uid_in_slip !== null ? (
                                                    <Box
                                                        ...
                                                        onClick={() => {
                                                            navigate(
                                                                `/admin/slip/${city.code}/${filial.eais}/${item.uid_in_slip}/?${wp !== null ? 'wp=' + wp : ''}`
                                                            )
                                                        }}
                                                    >
                                                        Слип приход
                                                ...
                                                {item.uid_out_receipt !== null ? (
                                                    <Box
                                                        ...
                                                        onClick={() => {
                                                            navigate(
                                                                `/admin/receipt/${city.code}/${filial.eais}/${item.uid_out_receipt}/?${wp !== null ? 'wp=' + wp : ''}`
                                                            )
                                                        }}
                                                    >
                                                        Чек возврат
                                                ...
                                                {item.uid_out_slip !== null ? (
                                                    <Box
                                                        ...
                                                        onClick={() => {
                                                            navigate(
                                                                `/admin/slip/${city.code}/${filial.eais}/${item.uid_out_slip}/?${wp !== null ? 'wp=' + wp : ''}`
                                                            )
                                                        }}
                                                    >
                                                        Слип возврат
    ...
    return (
        <Box
            ...
            className={
                (props.type === 'cinema' && pre_order_paying) || (props.type === 'horeca' && horder_paying)
                    ? 'payment-paying'
                    : null
            }
        >
            <Box className="payment-total">
                <Box className="payment-total-div">
                    ...
                    <Box sx={{backgroundColor: '#e4e2e2'}}>
                        <Box className="payment-total-title">Кино</Box>
                        <Box className="payment-total-sum">
                            {Math.round(pre_order.sum).toLocaleString('ru-RU')}
                        </Box>
                    </Box>
                    <Box>
                        <Box className="payment-total-title">Общепит</Box>
                        <Box className="payment-total-sum">
                            {Math.round(horder.sum).toLocaleString('ru-RU')}
                        </Box>
                    </Box>
                    <Box sx={{backgroundColor: '#e4e2e2'}}>
                        <Box className="payment-total-title">Всего</Box>
                        <Box className="payment-total-sum">{Math.round(total).toLocaleString('ru-RU')}</Box>
                    </Box>
                    <Box
                        sx={{cursor: 'pointer'}}
                        onClick={() => dispatch(openModal({type: 'calc', props: {}}))}
                    >
                        <Box className="payment-total-title">Получил</Box>
                        <Box className="payment-total-sum">{Math.round(cash).toLocaleString('ru-RU')}</Box>
                    </Box>
                    <Box
                        style={{
            ...
            <Box className="payment-types">
                {!show_payment_types ? (
                    ...
                    <Loader size={2}/>
                ) : payment_methods.error !== null ? (
                    <Box sx={{color: '#ff1a25', fontWeight: 'bold'}}>Ошибка загрузки маршрутов оплаты</Box>
                ) : payment_methods.data === null || payment_methods.data.list.length === 0 ? (
                    <Box sx={{color: '#ff1a25', fontWeight: 'bold', textAlign: 'center'}}>
                        Для этого рабочего места не найдено маршрутов оплаты, обратитесь в учетный отдел
                    </Box>
                ) : (
                    payment_methods.data.list.map((pm) => {
                                            ...
                                            if (pm.hidden) {
                                                ...
                                                if (payment_group[chapter0][chapter1][chapter2].items.includes(item.uid)) {
                                                    if (item.name_payment_type === null) {
                                                        if (
                                                            ['Сервис', 'Отложенная оплата', 'Смешанная'].includes(
                                                                item.name_payment_type
                                                            )
                                                        ) {
                                                            ok = false
                                                        }
                                                        ...
                                                        if (
                                                            ['СБП', 'Безналичные'].includes(item.name_payment_type) &&
                                                            pm.name !== 'Безналичные'
                                                        ) {
                                                            ok = false
                                                        } else if (
                                                            ['Наличные'].includes(item.name_payment_type) &&
                                                            pm.name !== 'Наличные'
                                                        ) {
                                                            ok = false
                                                        } else if (
                                                            ['Безналичные (б/т)'].includes(item.name_payment_type) &&
                                                            pm.name !== 'Безналичные (б/т)'
                                                        ) {
                                                            ok = false
                                                        } else if (
                                                            ['На расчетный счет'].includes(item.name_payment_type) &&
                                                            pm.name !== 'На расчетный счет'
                                                        ) {
                                                            ok = false
                                                        } else if (
                                                            ['Сервис', 'Отложенная оплата', 'Смешанная'].includes(
                                            ...
                                            if (pm.hidden) {
                                                ...
                                                if (payment_group[chapter0][chapter1][chapter2].items.includes(item.uid)) {
                                                    if (item.name_payment_type === null) {
                                                        ...
                                                        if (
                                                            ['Безналичные', 'СБП'].includes(item.name_payment_type) &&
                                                            pm.name !== 'Безналичные'
                                                        ) {
                                                            ok = false
                                                        } else if (
                                                            ...
                                                            [
                                                                ...
                                                                'На расчетный счет',
                                                                'Смешанная',
                                                            ].includes(item.name_payment_type)
                                                        ) {
                                                            ok = false
                                                        }
                                                    }
                      ...
                      <div>ККТ ...{pm.kkt.number.slice(-4)}</div>
                                            {pm.pinpad !== null ? (
                                                <div>Пинпад ...{pm.pinpad.number.slice(-4)}</div>
                                            ) : null}
                    </span>
                                    </div>
                                </Button>
                ...
                {show_payment_types &&
                    !payment_methods.loading &&
                    payment_methods.error === null &&
                    payment_methods.data !== null &&
                    payment_methods.data.list.length > 0 && (
                        <Button
            ...
            <Box className="payment-items">
                <Fade
                    in={payment_group.for_payment.slip_without_receipt.count > 0}
                    timeout={500}
                    unmountOnExit
                >
                    <Box className="payment-items-group">
                        <Box
                            className="payment-items-group-title"
                            sx={{
                                backgroundColor: '#f74b53',
                                color: 'white',
                            }}
                        >
                            {PAYMENT_STATE_SLIP_WITHOUT_RECEIPT}
                        ...
                        <GroupedTable
                            group={props.order.for_payment.slip_without_receipt.mark_egais_items}
                            title={ITEMS_TYPE_MARK_EGAIS}
                            chapter={'for_payment.slip_without_receipt.mark_egais_items'}
                        />
                        <GroupedTable
                            group={props.order.for_payment.slip_without_receipt.horeca_items}
                            title={ITEMS_TYPE_ITEMS}
                            chapter={'for_payment.slip_without_receipt.horeca_items'}
                        />
                        <GroupedTable
                            group={props.order.for_payment.slip_without_receipt.cinema_items}
                ...
                <Fade in={payment_group.for_payment.waiting.count > 0} timeout={500} unmountOnExit>
                    <Box className="payment-items-group">
                        ...
                        <GroupedTable
                            group={props.order.for_payment.waiting.mark_egais_items}
                            title={ITEMS_TYPE_MARK_EGAIS}
                            chapter={'for_payment.waiting.mark_egais_items'}
                        />
                        <GroupedTable
                            group={props.order.for_payment.waiting.horeca_items}
                            title={ITEMS_TYPE_ITEMS}
                            chapter={'for_payment.waiting.horeca_items'}
                        />
                        <GroupedTable
                            group={props.order.for_payment.waiting.cinema_items}
                            title={ITEMS_TYPE_SERVICE}
                ...
                <Fade
                    in={payment_group.for_returning.slip_without_receipt.count > 0}
                    timeout={500}
                    unmountOnExit
                >
                    <Box className="payment-items-group">
                        ...
                        <GroupedTable
                            group={props.order.for_returning.slip_without_receipt.mark_egais_items}
                            title={ITEMS_TYPE_MARK_EGAIS}
                            chapter={'for_returning.slip_without_receipt.mark_egais_items'}
                        />
                        <GroupedTable
                            group={props.order.for_returning.slip_without_receipt.horeca_items}
                            title={ITEMS_TYPE_ITEMS}
                            chapter={'for_returning.slip_without_receipt.horeca_items'}
                        />
                        <GroupedTable
                            group={props.order.for_returning.slip_without_receipt.cinema_items}
                            title={ITEMS_TYPE_SERVICE}
                ...
                <Fade in={payment_group.for_returning.waiting.count > 0} timeout={500} unmountOnExit>
                    <Box className="payment-items-group">
                        ...
                        {return_reasons_list()}
                        <GroupedTable
                            group={props.order.for_returning.waiting.mark_egais_items}
                            title={ITEMS_TYPE_MARK_EGAIS}
                            chapter={'for_returning.waiting.mark_egais_items'}
                        />
                        <GroupedTable
                            group={props.order.for_returning.waiting.horeca_items}
                            title={ITEMS_TYPE_ITEMS}
                            chapter={'for_returning.waiting.horeca_items'}
                        />
                        <GroupedTable
                            group={props.order.for_returning.waiting.cinema_items}
                            title={ITEMS_TYPE_SERVICE}
                ...
                <Fade in={payment_group.for_returning.success.count > 0} timeout={500} unmountOnExit>
                    <Box className="payment-items-group">
                        ...
                        <GroupedTable
                            group={props.order.for_returning.success.mark_egais_items}
                            title={ITEMS_TYPE_MARK_EGAIS}
                            chapter={'for_returning.success.mark_egais_items'}
                        />
                        <GroupedTable
                            group={props.order.for_returning.success.horeca_items}
                            title={ITEMS_TYPE_ITEMS}
                            chapter={'for_returning.success.horeca_items'}
                        />
                        <GroupedTable
                            group={props.order.for_returning.success.cinema_items}
                            title={ITEMS_TYPE_SERVICE}
