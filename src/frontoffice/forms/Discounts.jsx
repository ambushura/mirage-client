...
const Discounts = (props) => {
    ...
    useEffect(() => {
        const fetch = async () => {
            const fetching_result_discounts = await dispatch(
                cinema_seance_discounts_get(filial, seance.uid)
            )
            ...
            const fetching_result_discounts_groups = await dispatch(
                cinema_seance_discounts_groups_get(filial)
            )
    ...
    return (
        ...
        >
            ...
            <Box sx={{display: 'flex', flexDirection: 'column', maxWidth: '300px', overflow: 'hidden'}}>
                <FormControl variant="filled" sx={{m: 1, minWidth: '200px'}}>
                    ...
                    <Select
                        ...
                        variant="filled"
                    >
                        {discounts_groups.map((discount_group) => (
                            <MenuItem sx={{color: 'black'}} key={discount_group.uid} value={discount_group.uid}>
                                {discount_group.name}
                            </MenuItem>
                        ))}
                    </Select>
                ...
                <FormControl variant="filled" sx={{m: 1, minWidth: '200px'}} error={discount_error}>
                    ...
                    >
                        {discounts.map((discount) => (
                            <MenuItem sx={{color: 'black'}} key={discount.uid} value={discount.uid}>
                                {discount.name}
                                <span
                                    style={{
                                        padding: '4px 8px',
            ...
            <Button sx={{m: 1, minWidth: '200px'}} variant="contained" color="secondary" type="submit">
                Сохранить
            </Button>
    </Box>
    )
