...
export function useSetWS() {
    ...
    const wsUrl = useMemo(() => {
        ...
        const base = dev
            ? `ws://${ROUTE_MAIN_HOST.ip}:${ROUTE_MAIN_HOST.ws_port}`
            : `ws://${filial.ip}:${ROUTE_MAIN_HOST.ws_port}/ws`
        ...
        try {
            ...
            switch (data.type) {
                ...
                case 1:
                    if (!filial) {
                        dispatch(
                            addNotification({message: 'Выберите филиал', severity: 'error', autoHide: true})
                        )
                        return
                    }
                        ...
                        case 0:
                            if (uid_user === null && !kiosk) {
                                ...
                                if (['admin/orders/cinema', 'admin/orders/horeca'].includes(current_page)) {
                                    ...
                                    if (horder.in_base) {
                                        dispatch(
                                            pl_estimate_discounts(filial, horder.uid, 'horeca', data.value, horder.ver)
                                        )
                                    }
                                }
                            }
                ...
                case 'cinema':
                    if (
                        data.action === 'position_add' &&
                        current_page === 'seance' &&
                        seance.uid === data.uid_seance
                    ) {
                        ;(async () => {
                            const fetching_result = await dispatch(
                                cinema_seance_booking_get(filial, data.uid_seance, pre_order.uid, false)
                            )
                ...
                case 'kitchen':
                    if (
                        ...
                        param_date === dayjs(data.date_shift).format('YYYY-M-D')
                    ) {
                        ;(async () => {
                            const fetching_result = await dispatch(
                                horeca_kitchen_order_get(
                        ...
                        switch (data.action) {
                            case 'comment':
                                dispatch(
                                    addNotification({
                                        message: `${data.number} · КОММЕНТАРИЙ`,
                                        severity: 'info',
                                        autoHide: true,
                                    })
                                )
                            ...
                            case 'comment_deleted':
                                dispatch(
                                    addNotification({
                                        message: `${data.number} · УДАЛЕН КОММЕНТАРИЙ`,
                                        severity: 'info',
                                        autoHide: true,
                                    })
                                )
                            ...
                            case 'quantity_changed':
                                dispatch(
                                    addNotification({
                                        message: `${data.number} · КОЛИЧЕСТВО`,
                                        severity: 'info',
                                        autoHide: true,
                                    })
                                )
                            ...
                            case 'cook_2':
                                dispatch(
                                    addNotification({
                                        message: `${data.number} · НАЧНИТЕ ГОТОВИТЬ`,
                                        severity: 'info',
                                        autoHide: true,
                                    })
                                )
                            ...
                            case 'cook_3':
                                dispatch(
                                    addNotification({
                                        message: `${data.number} · ОТДАЙТЕ ГОСТЮ`,
                                        severity: 'info',
                                        autoHide: true,
                                    })
                                )
                            ...
                            case 'push':
                                dispatch(
                                    addNotification({
                                        message: `${data.number} · ГОТОВНОСТЬ`,
                                        severity: 'info',
                                        autoHide: true,
                                    })
                                )
                            ...
                            case 'delete_position':
                                dispatch(
                                    addNotification({
                                        message: `${data.number} · отмена позиции`,
                                        severity: 'info',
                                        autoHide: true,
                                    })
                                )
