...
const SsSchedule = ({width, height}) => {
    ...
    function binPack(rects, binWidth, binHeight) {
        ...
        for (let r of rects) {
            ...
            spaces.splice(idx, 1)
            spaces.push({
                x: space.x + r.w, y: space.y, w: space.w - r.w, h: r.h
            })
            spaces.push({
                x: space.x, y: space.y + r.h, w: space.w, h: space.h - r.h
            })
        }
        return result
    }
    ...
    let rects = baseRects.map(r => ({
        w: Math.ceil(r.baseW * scale), h: Math.ceil(r.baseH * scale), film: r.film
    }))
    ...
    const baseSeanceFont = {
        hall: maxCardHeight * 0.04, time: maxCardHeight * 0.08
    }
