            ...
            for (i; i < 2; i++) {
                top_menu[i].forEach(old_option => {
                    ...
                    if (PARAM_DATE_SHIFT.find(el => el === new_option.id) !== undefined) {
                        new_option.path = `/${old_option.id}/${city.code}/${filial === undefined ? 'all' : filial.eais}/${param_date}?${wp !== null ? 'wp=' + wp : ''}${kiosk ? '&kiosk' : ''}`
                    }
                    if (PARAM_DATE_SHIFT.find(el => el === new_option.id) === undefined) {
                        if (new_option.id.includes('admin')) {
                            new_option.path.forEach(el => {
                                el.path = `/${el.id}/${city.code}/${filial === undefined ? 'all' : filial.eais}/${param_date_admin}?${wp !== null ? 'wp=' + wp : ''}${kiosk ? '&kiosk' : ''}`
                            })
                        } else if (new_option.id === 'kitchen') {
                            new_option.path = `/${new_option.id}/${city.code}/${filial === undefined ? 'all' : filial.eais}/${param_date_admin}?${wp !== null ? 'wp=' + wp : ''}${kiosk ? '&kiosk' : ''}`
                        } else {
                            new_option.path = `/${old_option.id}/${city.code}/${filial === undefined ? 'all' : filial.eais}?${wp !== null ? 'wp=' + wp : ''}${kiosk ? '&kiosk' : ''}`
                        }
                    }
